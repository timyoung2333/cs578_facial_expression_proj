...
if __name__ == "__main__":
    ...
    # Sample code
    fer = FER2013()
    img_ids = ["{:05d}".format(i) for i in range(10000)]

    import random
    random.shuffle(img_ids)
    X_train, y_train = fer.getSubset(img_ids[:8000], encoding="raw_pixels")
    X_test, y_test = fer.getSubset(img_ids[8000:], encoding="raw_pixels")
